...
class App extends Component {
  ...
  addCartItem = product => {
    ...
    const productFound = cartList.find(eachItem => eachItem.id === product.id)
    if (productFound) {
      this.setState(prevState => ({
        cartList: prevState.cartList.map(eachItem => {
          if (product.id === eachItem.id) {
            const updateItemQnt = eachItem.quantity + product.quantity
            return {...eachItem, quantity: updateItemQnt}
          }
          return eachItem
        }),
      }))
  ...
  incrementCartItemQuantity = id => {
    this.setState(prevState => ({
      cartList: prevState.cartList.map(eachItem => {
        if (eachItem.id === id) {
          const updateItemQnt = eachItem.quantity + 1
          return {...eachItem, quantity: updateItemQnt}
        }
        return eachItem
      }),
